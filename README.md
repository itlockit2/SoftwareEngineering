...
![Alt text](/14.jpg)
## 9.6 시스템 및 인수테스트
전체 시스템이 기능적 요구와 비기능적 요구를 만족하는지 확인하는 테스트이다.
### 9.6.1 기능테스트
기능테스트는 기능적 요구와 시스템의 차이를 발견하는 것이다.
유즈케이스 모형에서 추출하여 테스트를 하며 사용사례기반 테스팅 처럼 모든 유즈케이스에 대해서 테스트하는것이 아닌
액터와의 인터액션들 중 오류를 일으키기 쉬워 보이는것만 테스트 한다.
### 9.6.2 성능테스트
성능테스트는 시스템의 여려측면, 작업 부하(Work load), 처리량(throughput), 반응시간(response time), 효율성 등을 평가하기 위한 목적이다.
### 9.6.3 보안테스트
시스템과 사용자에게 해를 입히려는 해커에게 이용될 수 있는 의도하지 않는 기능과 동작을 찾아내려는 것 보다 이 사이의 차이를 메우려는 작업이다.
즉 시스템의 보안 약점을 이용할 수 있는 취약점을 찾아내려는 목적이 있다.
### 9.6.4 사용자 인터페이스 테스트
1. 보고 느끼는 UI에 대한 결함
2. 데이터 입력과 출력 디스플레이에 대한 결함
3. 액터-시스템 사이의 동작 결함
4. 오류 처리에 대한 결함
5. 문서와 도움말에 대한 결함
### 9.6.5 인수테스트
인수테스트는 개발자가 하지 않고 개발을 의뢰한 사람이나 대리인이 한다.
인수 테스트 방법의 대부분은 테스트와 같다. 다른 점은 사용자 환경에서 한다는 것이다.
## 9.7 테스트도구
### 9.7.1 코드 분석 도구
코드 분석 도구는 프로그램을 실행하지 않고 분석하는 정적(static analysis)도구와 프로그램이 실행되면서 분석하는 동적 분석(dynamic analysis)가 있다.
* 정적 분석 도구
  * 코드 분석 도구
    * 문법이 틀렸다거나 오류가 포함될 가능성이 많은 위치를 표시해준다. 
  * 구조 검사 도구
    * 입력으로 제공된 원시코드의 그래프를 생성하여 논리 흐름을 보여주고 구조적 결함이 있는지 체크한다.
  * 데이터 분석 도구
    * 원시 코드에 정의된 데이터 구조, 데이터 선언, 컴포넌트 인터페이스를 검사하여 데이터 정의 충돌 등을 체크한다.
  * 순서 검사 도구
    * 이벤트의 순서를 체크해준다.
* 동적 분석 도구
동적 분석 도구는 프로그램이 수행되는 동안 이벤트의 상태를 파악하기 위해 특정한 변수나 조건의 스냅샷(Snapshot)을 생성한다.
기능에 영향을 주는 분기점(BreakPoint)를 정하여 그 값에 도달하면 그 때의 시스템의 상태, 변수의값이나 메모리 상태등을 알려준다.
### 9.7.2 테스트 케이스 생성 도구
1. 자료 흐름도
 * 원시 프로그램을 입력받아 파싱 한 후 자료 흐름도를 이용하여 define-use 관계를 찾아낸다.
2. 기능 테스트 방법
  * 주어진 기느을 구동시키는 모든 가능한 상태를 파악하여 이에 대한 입력을 작성
3. 입력 도메인 분석
  * 원시코드의 내부를 참조하지 않고 입력 변수가 가질 수 있는 값의 도메인을 분석하여 테스트 데이터 생성
4. 랜덤 테스트
  * 입력값을 무작위로 추출하여 테스트
### 9.7.3. 테스트 케이스 실행 도구
1. 캡쳐 및 리플레이
